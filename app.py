...
try:
    from incident_manager import get_active_incidents, submit_report, resolve_incident, init_db
except:
    pass
...
def calculate_status(risk_score):
    if risk_score <= 0.39:     return "Clear", "#28a745", 1.0
    elif risk_score <= 0.69:   return "Caution", "#ffc107", 1.4
...
trans = {
    "English": {
        ...
        "t1": "Dashboard & Tracking", "t2": "📱 Field Official Portal",
        ...
        "btn_reroute": "🚨 GLOBAL EMERGENCY REROUTE", "btn_reset": "Reset Fleet Positions",
    ...
    "हिंदी (Hindi)": {
        ...
        "t1": "डैशबोर्ड और ट्रैकिंग", "t2": "📱 फील्ड अधिकारी पोर्टल",
        ...
        "btn_reroute": "🚨 आपातकालीन मार्ग परिवर्तन", "btn_reset": "फ्लीट रीसेट करें",
        "orig": "मूल हब", "dest": "गंतव्य हब", "btn_preset": "त्वरित डेमो प्रीसेट (गुवाहाटी -> नोंगपोह)", "btn_calc": "लचीला मार्ग जांचे",
    ...
    "অসমীয়া (Assamese)": {
        ...
        "t1": "ডেছব'ৰ্ড আৰু ট্ৰেকিং", "t2": "📱 ক্ষেত্ৰ বিষয়া নিৱন্ধন",
        ...
        "btn_reroute": "🚨 জৰুৰী কালীন পথ সলনি", "btn_reset": "ফ্লীট ৰিছেট",
        "orig": "প্ৰাৰম্ভিক হাব", "dest": "গন্তব্য হাব", "btn_preset": "দ্ৰুত ডেমো (গুৱাহাটী -> নংপোহ)", "btn_calc": "বিকল্প পথ নিৰ্ধাৰণ কৰক",
        "m_tot": "মুঠ নেটৱৰ্ক", "m_clr": "যোগাযোগ মুকলি", "m_cau": "সতৰ্কতা / উচ্চ বিপদ", "m_blk": "বন্ধ / আবদ্ধ", "m_wea": "বৰ্তমান বতৰ",
        "trk_title": "লাইভ ফ্লীট ট্ৰেকিং",
        "tbl_col1": "গাড়ী ID", "tbl_col2": "সা-সৰঞ্জাম বস্তু", "tbl_col3": "অৱস্থা", "tbl_col4": "জৰুৰী কাৰ্য্য",
        ...
        "f_head": "ভূ-টেগ কৰা ঘটনাৰ প্ৰতিবেদন দাখিল কৰক", "f_sub1": "প্ৰতিবেদকৰ বিৱৰণ", "f_sub2": "স্থান আৰু শ্ৰেণীবিভাজন",
        "f_name": "প্ৰতিবেদকৰ নাম", "f_role": "পদবী / সংস্থা", "f_type": "ঘটনাৰ প্ৰকাৰ", "f_sev": "গুৰুত্ব", "f_seg": "প্ৰভাৱিত পথ", "f_desc": "ঘটনাৰ টোকা", "f_photo": "ফটো আপলোড কৰক", "btn_sub": "প্ৰতিবেদন দাখিল কৰক 🚀",
        "suc": "ঘটনা সফলতাৰে লিপিৱদ্ধ কৰা হ'ল!", "suc_d": "ডেছব'ৰ্ড মানচিত্ৰ স্বয়ংক্ৰিয়ভাৱে আপডেট হৈছে।"
    }
}
t = trans[lang]
...
st.title(t["title"])
st.sidebar.markdown('---')
st.sidebar.markdown("🟢 **Edge Node Status: Online**", help="Running on Local SQLite Cached Mode. Field updates queue locally and will seamlessly auto-sync to Delhi Central Servers upon restoring network coverage.")

try:
    incidents_df = get_active_incidents()
except:
...
if not incidents_df.empty and "severity" in incidents_df.columns:
    field_severed = incidents_df[incidents_df["severity"] == "Complete Road Severed"]["nearest_segment_id"].tolist()
    field_blocks.extend(field_severed)
...
guwahati_preset = all_nodes[0] if len(all_nodes) > 0 else ""
nongpoh_preset = all_nodes[-1] if len(all_nodes) > 0 else ""
node_options = [f"Node {n}" for n in all_nodes]
...
origin_node = str(origin_sel).replace("Node ", "") if not b_preset else str(guwahati_preset)
dest_node = str(dest_sel).replace("Node ", "") if not b_preset else str(nongpoh_preset)
...
with tabs[0]:
    ...
    def update_row(row):
        ...
        if row["segment_id"] in forced_blocks:
            status, color, delay = "Blocked", "#dc3545", 999.0
            rs = 1.0
        row["risk_score"] = round(rs, 3)
        row["status"] = status
        row["color"] = color
        row["delay_factor"] = delay
        return row
    roads = roads.apply(update_row, axis=1)

    for i, (u, v, k, d) in enumerate(G.edges(keys=True, data=True)):
        if i < len(roads):
            d["status"] = roads.iloc[i]["status"]
    ...
    col1.metric(t["m_tot"], f"{roads['length_km'].sum():.1f} km")
    col2.metric(t["m_clr"], len(roads[roads["status"] == "Clear"]))
    col3.metric(t["m_cau"], len(roads[roads["status"] == "Caution"]))
    col4.metric(t["m_blk"], len(roads[roads["status"] == "Blocked"]))
    col5.metric(t["m_wea"], f"{current_precip:.1f} mm/h (Sim: {sim_rain:.1f})")

    active_alerts = [v for v in fleet_status if v["alert"]]
    if active_alerts:
        st.markdown("---")
        for v in active_alerts:
            st.error(f"**🚨 AUTOMATED DISPATCH ALERT: {v['id']}**\n\n{v['advisory']}")

    st.markdown("---")
    st.subheader(t["trk_title"])
    if fleet_status:
        table_data = [{t["tbl_col1"]: v["id"], t["tbl_col2"]: v["cargo"], t["tbl_col3"]: v["status"], t["tbl_col4"]: v["advisory"]} for v in fleet_status]
    ...
    if b_route or b_preset:
        ...
        if res["status"] == "IMPASSABLE":
            ...
            st.warning(f"⚠️ Primary highway compromised at {len(res['compromised_segments'])} segments. Active detour deployed.")
            c1, c2 = st.columns(2)
            with c1: st.info(f"**Baseline Route (Historical)**\n\nDistance: {res['base_dist_km']} km\n\nTime: {res['base_duration_min']} mins")
            with c2: st.success(f"**Resilient Detour (Active)**\n\nDistance: {res['res_dist_km']} km\n\nTime: {res['res_duration_min']} mins\n\n{res['detour_delay_message']}")
    ...
    if route_metrics and route_metrics.get("status") == "SUCCESS":
        ...
        if route_metrics["resilient_path"]:
            ...
            folium.Marker(res_coords[0], popup="Origin Hub", icon=folium.Icon(color="green")).add_to(m)
            folium.Marker(res_coords[-1], popup="Destination Hub", icon=folium.Icon(color="red")).add_to(m)
    ...
    if not incidents_df.empty:
        for _, row in incidents_df.iterrows():
            popup_html = f"<b>{row['incident_type']}</b><br>Reporter: {row['reporter_name']}<br>Severity: {row['severity']}<br>Notes: {row['description']}"
            if row["photo_path"] and os.path.exists(row["photo_path"]):
                with open(row["photo_path"], "rb") as image_file: popup_html += f"<br><br><img src='data:image/jpeg;base64,{base64.b64encode(image_file.read()).decode()}' width='200'>"
    ...
    if not incidents_df.empty:
        for idx, row in incidents_df.iterrows():
            ...
            if c2.button("Resolve Incident ✓", key=f"res_{row['id']}"):
